# TypeRef Features Summary

## Overview
The `TypeRef<TBase>` system provides a powerful way to select and construct types at edit-time in Unity, with full support for generic type construction including infinite nesting depth.

## Core Features

### 1. **Basic Type Selection**
- Select any concrete type that implements the base interface
- Example: `TypeRef<IDamageEffect>` shows all concrete damage types

### 2. **Generic Type Construction**
- Enable with `[TypeRefOptions(includeGenericTypeDefinitions: true)]`
...
- **Enabled**: `[TypeRefOptions(allowSelfNesting: true)]`
...
- **Enabled**: `[TypeRefOptions(allowOpenGenerics: true)]`
...
[TypeRefOptions(ExcludeTypes = new[] { typeof(FireDamage), typeof(IceDamage) })]

// Exclude via resolver method
[TypeRefOptions(ExcludeTypesResolver = "TypeName.MethodName")]
```

#### Inclusion Filters
```csharp
// Only show specific types
[TypeRefOptions(IncludeTypes = new[] { typeof(FireDamage), typeof(IceDamage) })]

// Include via resolver method
[TypeRefOptions(IncludeTypesResolver = "TypeName.MethodName")]
```
...
[TypeRefOptions(ExcludeTypesResolver = "MyClass.GetElementalTypes")]
TypeRef<IDamageEffect> myField;
```
...
The `TypeRefExample.cs` file includes comprehensive test cases:
...
[TypeRefOptions(
    includeGenericTypeDefinitions: true,
    IncludeTypesResolver = "GetContainerTypes"
)]
TypeRef<IDataContainer> containerType;
```
