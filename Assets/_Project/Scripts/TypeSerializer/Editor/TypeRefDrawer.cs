...
namespace Editor {
    ...
    /// Supports constructing generic types by selecting type arguments.
    /// </summary>
    public sealed class TypeRefDrawer<TBase> : OdinValueDrawer<TypeRef<TBase>> where TBase : class {
        TypeRefOptionsAttribute? options;
        List<GenericSelectorItem<Type>>? dropdownItems;
        List<Type>? availableTypes;
        bool initialized;
        
        // For generic type construction - Unified recursive approach
        Type[]? selectedTypeArguments; // Root level type arguments
        GenericConstructionState? constructionState; // Manages nested construction state
        ...
        protected override void DrawPropertyLayout(GUIContent label) {
            ...
            // We're constructing if we have an open generic type AND selectedTypeArguments is initialized
                ...
                selector.SelectionConfirmed += selection => {
                    ...
                    if (selectedType != null && selectedType.IsGenericTypeDefinition) {
                        if (allowOpenGenerics && !allowGenericTypeConstruction) {
                            ...
                            // Neither option is enabled - this shouldn't happen if RefreshDropdownList is correct
        ...
        void DrawGenericTypeConstructor(GUIContent label, Type openGenericType) {
            ...
            if (selectedTypeArguments == null || selectedTypeArguments.Length != genericArgs.Length) {
                ...
                var newArray = new Type?[genericArgs.Length];
                
                // Preserve existing arguments if resizing
                if (selectedTypeArguments != null) {
                    for (int i = 0; i < Math.Min(selectedTypeArguments.Length, newArray.Length); i++) {
                        newArray[i] = selectedTypeArguments[i];
                    }
                }
                
                selectedTypeArguments = newArray;
            }
            ...
            if (allowOpenGenerics) {
                ...
                // e.g., MyType<int, List<T>> is valid
                GUI.enabled = allArgsSelected; // Can only construct if all arguments are selected
                
                // Visual feedback: use different color when type contains generic parameters
                var originalColor = GUI.backgroundColor;
                if (!isFullyConcrete) {
                    GUI.backgroundColor = new Color(0.8f, 0.6f, 1f); // Purple tint for types with generic parameters
                }
                
                string buttonLabel = isFullyConcrete ? "Construct Type" : "Construct Type (w/ Open)";
                if (GUILayout.Button(buttonLabel)) {
                    // Construct with the selected arguments (may include open generics)
            ...
            if (GUILayout.Button("Cancel")) {
                if (allowOpenGenerics) {
                    ...
                    // The type is already set, so we just clear the construction state
                }
                else {
        ...
        /// <summary>
        /// Builds a preview string showing what the type would look like with current selections.
        /// </summary>
        ...
        /// <summary>
        /// Recursively builds preview string for nested types.
        /// </summary>
        ...
        /// <summary>
        /// Checks if the given path represents the deepest expanded constructor.
        /// This ensures we only show Apply/Cancel buttons on the most nested constructor.
        /// </summary>
        bool IsDeepestExpandedConstructor(List<int> path) {
            ...
            var args = constructionState.GetArguments(path);
            if (args == null)
                return true; // No children, so this is the deepest
            
            var expandedIndex = constructionState.GetExpandedIndex(path);
            if (expandedIndex.HasValue) {
                // This path has an expanded child, so it's not the deepest
                return false;
            }
            ...
            for (int i = 0; i < args.Length; i++) {
                if (args[i] != null && args[i]!.IsGenericTypeDefinition) {
                    ...
                    if (childExpandedIndex.HasValue) {
                        // A child is expanded, so this is not the deepest
                        return false;
                    }
                }
            }
            
            return true; // No children are expanded, so this is the deepest
        }
        
        /// <summary>
        /// Applies nested construction from a child path. This takes the constructed (or open generic)
        /// type from the nested constructor and assigns it to the parent's argument.
        /// When AllowOpenGenerics is true, the constructed type can contain open generic type definitions as arguments.
        /// </summary>
        void ApplyNestedConstruction(List<int> parentPath, int argIndex) {
            ...
            try {
                // Construct the nested type with its arguments
                // The arguments can be concrete types OR open generic type definitions (when AllowOpenGenerics is true)
                var constructedNested = nestedGenericType.MakeGenericType(childArgs.Cast<Type>().ToArray());
                
                // Determine if the constructed type contains any open generics
                // Check if the constructed type itself contains generic parameters (e.g., Container<List<T>> contains T)
                bool containsOpenGenerics = constructedNested.ContainsGenericParameters;
                
                // When AllowOpenGenerics is false, we cannot apply if the result contains open generics
        ...
        void ApplyConstruction(Type openGenericType, Type?[]? typeArgs) {
            ...
            try {
                // Process all arguments - each must be either fully concrete or an open generic definition
                ...
                // Check if we can construct based on AllowOpenGenerics setting
        ...
        /// <summary>
        /// Result of drawing a generic type constructor.
        /// </summary>
        struct ConstructionResult {
            public bool IsFullyConcrete; // True if all type arguments are concrete (no open generics anywhere)
            public bool AllArgumentsSelected; // True if all type arguments have been selected (may include open generics)
        }
        
        /// <summary>
        /// Recursively draws generic type constructor UI for any nesting depth.
        /// </summary>
        /// <param name="openGenericType">The generic type definition to construct</param>
        /// <param name="path">The path to this constructor (list of argument indices)</param>
        /// <param name="depth">The current nesting depth (for indentation)</param>
        /// <param name="targetArgsArray">The array to store the final constructed type in</param>
        /// <returns>Construction result indicating completeness and selection status</returns>
        ConstructionResult DrawGenericConstructorRecursive(Type openGenericType, List<int> path, int depth, Type?[]? targetArgsArray) {
            ...
            if (isNested) {
                EditorGUILayout.BeginHorizontal();
                GUILayout.Space(15); // Fixed indent amount per level
            ...
            var expandedIndex = constructionState.GetExpandedIndex(path);
            bool allArgsConcrete = true; // True if all arguments are concrete (no open generics)
            bool allArgsSelected = true; // True if all arguments have been selected
            ...
            for (int i = 0; i < genericArgs.Length; i++) {
                ...
                if ((expandedIndex == i || shouldAutoExpand) && currentArg != null && currentArg.IsGenericTypeDefinition) {
                    ...
                    // When AllowOpenGenerics is false, don't show Apply/Cancel for nested constructors
                    // They'll all be constructed together at the root level
                    // Also ONLY show Apply/Cancel for the deepest nested constructor to avoid multiple buttons
                    ...
                    if (shouldShowApplyCancel) {
                        ...
                        if (GUILayout.Button(applyLabel, GUILayout.Width(110))) {
                            // Apply the nested construction
                            // This constructs the type with selected arguments (which may include open generic definitions)
                        ...
                        if (GUILayout.Button("Cancel", GUILayout.Width(60))) {
                            ...
                            args[i] = null; // Clear the selection
                            constructionState.SetArguments(path, args);
                            GUIHelper.RequestRepaint();
                        }
                    ...
                    // Even if the child is not fully concrete, we can still mark this arg as "selected"
                    // because it will be a constructed type (even if it contains open generics)
                    if (!childResult.AllArgumentsSelected) {
                        // Child is missing some arguments, so we can't apply it yet
                        allArgsSelected = false;
                    }
                ...
                if (currentArg != null && (currentArg.IsGenericTypeDefinition || currentArg.ContainsGenericParameters)) {
                    // The type contains generic parameters - either an open definition (List<>) or a partially constructed type (Container<List<T>>)
                    allArgsConcrete = false;
                    
                    if (allowOpenGenerics) {
                        // When AllowOpenGenerics is true, show a Construct button to optionally construct/re-construct it
                        // Visual feedback: use different color for types that already contain some generic parameters (not just open definitions)
                        var originalColor = GUI.backgroundColor;
                        ...
                        if (isPartiallyConstructed) {
                            GUI.backgroundColor = new Color(0.8f, 0.6f, 1f); // Purple tint for partially constructed types
                        }
                    ...
                    // When AllowOpenGenerics is false, these types will be auto-expanded for construction (handled earlier in the code)
                }
        ...
        void ShowTypeArgumentSelectorRecursive(List<int> path, int argIndex, Type genericParameter, Type?[] targetArgsArray) {
            ...
            var validTypes = allTypes
                ...
                .Where(t => {
                    ...
                    // Check if open generics are allowed as final result
                    if (!allowOpenGenerics && t.IsGenericTypeDefinition) {
                        // Still allow selection for construction purposes (they'll need to be constructed later)
                        // This is handled by the UI flow - user must click "Construct" button
                    }
                    ...
                    // (we can't verify they'll have a parameterless constructor when constructed)
        ...
        static string GetTypeName(Type type) {
            ...
            var argNames = string.Join(", ", genericArgs.Select(t => {
                if (t.IsGenericType) {
    ...
    sealed class GenericConstructionState {
        // Path is represented as a list of indices: [0, 2, 1] means root arg 0 -> nested arg 2 -> nested arg 1
