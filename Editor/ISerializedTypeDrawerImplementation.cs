...
namespace Hissal.UnityTypeSerializer.Editor {
    ...
    internal abstract class SerializedTypeDrawerBase {
        ...
        protected string GetSelectionDisplayName(Type? type) {
            return SerializedTypeDrawerCore.GetSelectionDisplayName(type, Accessor.GetSerializedTypeId(), Accessor.GetSerializedAqn());
        }
        ...
        protected void DrawTypeIdSyncWarningWithFixes(Type? selectedType) {
            var mismatches = Accessor.GetTypeTreeMismatches();
            if (mismatches.Length == 0)
                return;

            for (int i = 0; i < mismatches.Length; i++) {
                var mismatch = mismatches[i];
                EditorGUILayout.HelpBox(mismatch.WarningMessage, MessageType.Warning);

                if (!string.IsNullOrEmpty(mismatch.MatchTypeIdImpact)) {
                    EditorGUILayout.LabelField($"TypeId fix impact: {mismatch.MatchTypeIdImpact}", EditorStyles.wordWrappedMiniLabel);
                }

                if (!string.IsNullOrEmpty(mismatch.MatchAqnImpact)) {
                    EditorGUILayout.LabelField($"AQN fix impact: {mismatch.MatchAqnImpact}", EditorStyles.wordWrappedMiniLabel);
                }

                EditorGUILayout.BeginHorizontal();

                using (new EditorGUI.DisabledScope(!mismatch.CanMatchTypeId)) {
                    if (GUILayout.Button($"Fix Node Match TypeId ({mismatch.MatchTypeIdPreview})")) {
                        ApplySingleMismatchFix(mismatch.Path, SerializedTypeTreeFixMode.MatchTypeId);
                    }
                }

                using (new EditorGUI.DisabledScope(!mismatch.CanMatchAqn)) {
                    if (GUILayout.Button($"Fix Node Match AQN ({mismatch.MatchAqnPreview})")) {
                        ApplySingleMismatchFix(mismatch.Path, SerializedTypeTreeFixMode.MatchAqn);
                    }
                }
        ...
        void ApplySingleMismatchFix(string path, SerializedTypeTreeFixMode mode) {
            var previousType = Accessor.GetSelectedType();
            if (!Accessor.TryApplyTypeTreeMismatchFix(path, mode, out _))
                return;

            Accessor.ApplyChanges();
            var newType = Accessor.GetSelectedType();
