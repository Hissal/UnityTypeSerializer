...
namespace Hissal.UnityTypeSerializer.Editor {
    ...
    internal static class SerializedTypeDrawerCore {
        ...
        internal readonly struct SerializedTypeIdSyncFixes {
            public bool HasIssue { get; init; }
            public string WarningMessage { get; init; }
            public string MatchTypeIdTypeId { get; init; }
            public string MatchTypeIdAqn { get; init; }
            public string MatchTypeIdPreview { get; init; }
            public string MatchAqnTypeId { get; init; }
            public string MatchAqnAqn { get; init; }
            public string MatchAqnPreview { get; init; }
        }

        internal static bool HasInvalidSerializedType(Type? selectedType, string? serializedTypeId, string? serializedAqn) {
            return selectedType == null && (!string.IsNullOrEmpty(serializedTypeId) || !string.IsNullOrEmpty(serializedAqn));
        }

        internal static string GetSelectionDisplayName(Type? selectedType, string? serializedTypeId, string? serializedAqn) {
        ...
        internal static SerializedTypeIdSyncFixes GetTypeIdSyncFixes(
            ...
            var normalizedSerializedTypeId = serializedTypeId ?? string.Empty;
            var normalizedSerializedAqn = serializedAqn ?? string.Empty;
            var selectedAqn = selectedType.AssemblyQualifiedName ?? string.Empty;
            SerializedTypeIdRegistry.TryGetTypeId(selectedType, out var selectedTypeId);

            bool typeIdMatchesSelected = string.Equals(normalizedSerializedTypeId, selectedTypeId, StringComparison.Ordinal);
            bool aqnMatchesSelected = string.Equals(normalizedSerializedAqn, selectedAqn, StringComparison.Ordinal);
            if (typeIdMatchesSelected && aqnMatchesSelected)
                return default;

            Type? typeFromId = null;
            bool hasTypeFromId = SerializedTypeIdRegistry.TryResolveType(normalizedSerializedTypeId, out typeFromId) && typeFromId != null;

            string matchTypeIdAqn = selectedAqn;
            string matchTypeIdTypeId = selectedTypeId;
            string matchTypeIdPreview = SerializedTypeDrawerUtilities.GetTypeName(selectedType);

            if (hasTypeFromId && typeFromId != null) {
                matchTypeIdAqn = typeFromId.AssemblyQualifiedName ?? string.Empty;
                SerializedTypeIdRegistry.TryGetTypeId(typeFromId, out matchTypeIdTypeId);
                matchTypeIdPreview = SerializedTypeDrawerUtilities.GetTypeName(typeFromId);
            }
            else {
                var aqnType = !string.IsNullOrEmpty(normalizedSerializedAqn) ? Type.GetType(normalizedSerializedAqn) : null;
                if (aqnType != null) {
                    matchTypeIdAqn = aqnType.AssemblyQualifiedName ?? string.Empty;
                    SerializedTypeIdRegistry.TryGetTypeId(aqnType, out matchTypeIdTypeId);
                    matchTypeIdPreview = SerializedTypeDrawerUtilities.GetTypeName(aqnType);
                }
            }

            string matchAqnAqn = normalizedSerializedAqn;
            string matchAqnTypeId = string.Empty;
            string matchAqnPreview = "None";

            var typeFromAqn = !string.IsNullOrEmpty(normalizedSerializedAqn) ? Type.GetType(normalizedSerializedAqn) : null;
            if (typeFromAqn != null) {
                matchAqnPreview = SerializedTypeDrawerUtilities.GetTypeName(typeFromAqn);
                SerializedTypeIdRegistry.TryGetTypeId(typeFromAqn, out matchAqnTypeId);
            }
            else if (!string.IsNullOrEmpty(normalizedSerializedAqn)) {
                matchAqnPreview = "Invalid";
            }

            return new SerializedTypeIdSyncFixes {
                HasIssue = true,
                WarningMessage =
                    "SerializedType id/AQN values are unsynced. Choose which source should be treated as canonical.",
                MatchTypeIdTypeId = matchTypeIdTypeId,
                MatchTypeIdAqn = matchTypeIdAqn,
                MatchTypeIdPreview = matchTypeIdPreview,
                MatchAqnTypeId = matchAqnTypeId,
                MatchAqnAqn = matchAqnAqn,
                MatchAqnPreview = matchAqnPreview
            };
        }
        ...
        internal static bool TryValidateSelectedType(
            ...
            if (selectedType == null) {
                if (HasInvalidSerializedType(selectedType, serializedTypeId, serializedAqn)) {
