...
- **Stable type identity**: Persist `typeId` values via `SerializedTypeIdAttribute` so references survive AQN/type renames
- **Analyzer and validator support**: Catch duplicate/missing IDs and fix ID/AQN drift from inspector tooling
...
- **Stable Serialized IDs**: Optional `[SerializedTypeId("...")]` support with runtime registry fallback
- **Mismatch Repair Tooling**: Inspector/Odin validation with `Match TypeId` and `Match AQN` repair actions
- **Roslyn Tooling**: Generator + analyzer + code-fix support for ID registration and diagnostics (`STG001`, `STG100`, `STG101`)
...
- `AllowedTypeKinds`: Control which type kinds appear (`Object`, `Class`, `Struct`, `Abstract`, `Interface`, etc.)
...
### `SerializedTypeIdAttribute`

Assigns a stable identifier to a type so serialized references can survive assembly-qualified-name churn.

```csharp
[SerializedTypeId("0f4ab3bb-4a90-4bd0-8c8c-31f9faef3f3f")]
public sealed class FireDamage : IDamageEffect { }
```

`SerializedType` resolves in this order at runtime:
1. Serialized type-tree (`typeId` + `aqn` + nested generic argument nodes)
2. Root `typeId` through `SerializedTypeIdRegistry`
3. Legacy root `aqn`

### Source Generation and Analyzer Pipeline

- Source project lives under `Assets/_Project/SourceGeneration/SerializedTypeGenerators~` (`~` keeps Unity from importing project sources)
...
|--------|------|-------------|
| `IsValid` | `bool` | Returns true if a valid type is set |
| `HasType` | `bool` | Obsolete alias of `IsValid` |
...
| `AllowedTypeKinds` | `SerializedTypeKind` | `Object` | Types to show in dropdown |
...
| Value | Description |
|-------|-------------|
| `Object` | Combined alias for `Class | Struct` |
| `Class` | Non-static, non-abstract class types |
| `Struct` | Non-primitive, non-enum structs |
| `Abstract` | Abstract classes |
| `Interface` | Interface types |

`Concrete` is kept as an obsolete alias of `Object` for backward compatibility.

Use bitwise OR to combine flags, for example: `Object | Abstract | Interface`.
...
6. **Runtime Usage**: Always check `IsValid` before accessing `Type` to avoid null reference issues.
